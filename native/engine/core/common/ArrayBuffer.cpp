...
ArrayBuffer::ArrayBuffer(unsigned int nSize, unsigned int nAdjustSize)
{
...
ArrayBuffer::ArrayBuffer(char *buffer, int nsize)
{
...
ArrayBuffer::ArrayBuffer(char *buffer, int nsize, bool bNeedDelBuffer)
{
...
ArrayBuffer::~ArrayBuffer()
{
    ...
    {
        // printf("destroy ArrayBuffer %p \n",m_pBuffer);
        delete[] m_pBuffer;
        m_pBuffer = 0;
    }
}
void ArrayBuffer::setBufferExpandStep(int nAdjustSize)
{
    m_nAdjustSize = nAdjustSize;
}
void ArrayBuffer::setAlign(bool bAlign)
{
    m_bAlign = bAlign;
}

char *ArrayBuffer::getBuffer()
{
    return m_pBuffer;
}

char *ArrayBuffer::getReadPtr()
{
...
unsigned int &ArrayBuffer::getCurrentPos()
{
    return m_nReadPos;
}

unsigned int ArrayBuffer::getDataSize()
{
    return m_nDataSize;
}

unsigned int ArrayBuffer::getBuffSize()
{
    return m_nBufSize;
}

void ArrayBuffer::setBufferSize(int nBufferSize)
{
    if (nBufferSize < 0)
    {
        return;
...
char *ArrayBuffer::readBuffer(int nLength)
{
...
char *ArrayBuffer::readBufferAlign(int nLength)
{
...
int ArrayBuffer::poppUTFStringLength()
{
    unsigned short &len = *popp<unsigned short>();
    return len + 1;
}

void ArrayBuffer::popUTFString(std::vector<unsigned int> &strs)
{

    unsigned short str_size = *popp<unsigned short>();
    unsigned int length = m_nReadPos + str_size;
...
void ArrayBuffer::popUTFString(std::vector<std::string> &strs)
{
    unsigned short str_size = *popp<unsigned short>();
    unsigned int length = m_nReadPos + str_size;
    unsigned char *c, *c2, *c3;
...
char *ArrayBuffer::popUTFString(char *strs, int length)
{
    // unsigned short& len = *popp<unsigned short>();
    // int b = m_nReadPos;
...
void ArrayBuffer::poppUTFStringg(std::string &str)
{
...
//--------------------------------
void ArrayBuffer::expand(int nExpandSize)
{
    if (nExpandSize < 1)
        return;
...
void ArrayBuffer::resize(int size, bool copy)
{
...
void ArrayBuffer::append(long v)
{
...
void ArrayBuffer::appendInt(int v)
{
    expand(m_nDataSize + sizeof(int) - m_nBufSize);
    *(int *)(m_pBuffer + m_nDataSize) = v;
    m_nDataSize += sizeof(int);
...
void ArrayBuffer::appendFloat(float v)
{
...
void ArrayBuffer::appendUnsignedShort(unsigned short v)
{
...
char *ArrayBuffer::append(const void *pBufffer, int nSize)
{
    if (!pBufffer)
        return NULL;
...
void *ArrayBuffer::appendEmpty(int nSize)
{
    expand(m_nDataSize + nSize - m_nBufSize);
...
void *ArrayBuffer::appendEmptyAlign(int nSize)
{
...
void ArrayBuffer::_setDataSize(int nSize)
{
    m_nDataSize = nSize;
}

void ArrayBuffer::setExternalBuffer(char *pBuffer, int nSize)
{
    if (m_bNeedDelBuffer && m_pBuffer)
    {
        delete[] m_pBuffer;
...
unsigned int ArrayBuffer::getReadPos()
{
    return m_nReadPos;
}

char *ArrayBuffer::getCurrentWriteBuffer()
{
