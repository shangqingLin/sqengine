...
namespace event
{
    ...
    /**
     * 使用 operator() 来判断是否为 lambda 表达式的关键在于，
     * lambda 表达式在 C++ 中是通过实现 operator() 来定义其调用行为的。
     * 因此，判断一个类型是否有 operator() 是识别 lambda 表达式和其他可调用对象（如函数对象）的一种有效方法。
     */
    template <typename F>
    ...
    struct TgtEvtFnTrait<R(C *, ARGS...)>
    {
        ...
        static auto wrap(src_func_type func)
        {
            ...
            else
            {
                ...
                ret = [func](emitter_type *self, event_type *event)
                {
                    return std::apply([self, func](auto &&...args)
                                      { func(self, args...); }, event->args);
                };
            }
            return ret;
        }
    ...
    struct TgtEvtFnTrait<R (C::*)(ARGS...)>
    {
        ...
        using param0_type = typename HeadType<ARGS...>::head;
        ...
        static auto wrapWithContext(src_func_type func, context_type *ctx)
        {
            ...
            else
            {
                ret = [func, ctx](emitter_type * /*self*/, event_type *event)
                {
                    return std::apply([ctx, func](auto... args)
                                      { (ctx->*func)(args...); }, event->args);
                };
            }
            return ret;
        }
    ...
    constexpr int TotalEvents = addAll(Parents::getEventCount()...);
}
