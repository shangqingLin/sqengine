...
namespace phxy
{
	...
	private:
		...
		public:
			...
			Slab(unsigned int numberOfItems) : m_numberOfItems(numberOfItems)
			{
				...
				SQ_ASSERT(sizeof(*this) == b2_mallocAlignment);
			}
			...
			unsigned int GetNumberOfItems() const { return m_numberOfItems; }

			/// Get a pointer to the first item in the slab.
			T *GetFirstItem() const
			{
				return (T *)((unsigned char *)(this + 1));
			}
		...
		private:
			...
			unsigned char m_padding[b2_mallocAlignment - sizeof(unsigned int)];
		};

	public:
		/// Initialize the allocator to allocate itemsPerSlab of type T for each
		/// slab that is allocated.
		b2SlabAllocator(const unsigned int itemsPerSlab) : m_itemsPerSlab(itemsPerSlab)
		{
		}
		...
		void SetItemsPerSlab(unsigned int itemsPerSlab)
		{
			m_itemsPerSlab = itemsPerSlab;
		}

		// Get the size of the next allocated slab.
		unsigned int GetItemsPerSlab() const
		{
		...
		bool AllocateSlab()
		{
			if (!m_itemsPerSlab)
				return false;
			const unsigned int slabSize = sizeof(Slab) + (sizeof(T) * m_itemsPerSlab);
			...
			for (unsigned int i = 0; i < m_itemsPerSlab; ++i, ++item)
			{
		...
		void FreeEmptySlabs()
		{
			const SqIntrusiveListNode &freeItemList =
				m_freeList.GetFreeList()->GetFreeList();
			const SqIntrusiveListNode *freeItemListTerminator =
			...
			while (block != slabListTerminator)
			{
				...
				const unsigned char *const slabItemStart = (unsigned char *)slab->GetFirstItem();
				const unsigned char *const slabItemEnd = (unsigned char *)slab->GetItemEnd();

				// Count all free items that are owned by the current slab.
				unsigned char freeItems = 0;
				bool empty = false;
				for (SqIntrusiveListNode *itemNode = freeItemList.GetNext();
					 itemNode != freeItemListTerminator;
					 itemNode = itemNode->GetNext())
				{
					const unsigned char *itemNodeAddress = (unsigned char *)itemNode;
		...
		const SqTypedFreeList<T> &GetFreeList() const
		{
			return m_freeList;
		}
	...
	private:
		...
		{
			SQ_ASSERT(slab);
			const unsigned int numberOfItems = slab->GetNumberOfItems();
			T *item = slab->GetFirstItem();
			for (unsigned int i = 0; i < numberOfItems; ++i, ++item)
			{
				item->~T();
			}
			slab->~Slab();
	...
	private:
		...
		unsigned int m_itemsPerSlab;
		/// Freelist which contains instances of T.
		SqTypedFreeList<T> m_freeList;
	};
