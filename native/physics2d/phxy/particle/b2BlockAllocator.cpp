
#include "b2BlockAllocator.h"
#include "../common/SqCore.h"
#include <limits.h>
#include <string.h>
#include <stddef.h>
#include <stdlib.h>

using namespace phxy;

//每个chunk的大小
static const int b2_chunkSize = 16 * 1024;

//每个chunk中
static const int b2_maxBlockSize = 640;

// 每次触发分配时，创建的b2Chunk的个数。
static const int b2_chunkArrayIncrement = 128;

// These are the supported object sizes. Actual allocations are rounded up the next size.
static const int b2_blockSizes[b2_blockSizeCount] =
	{
		16,	 // 0
		32,	 // 1
		64,	 // 2
		96,	 // 3
		128, // 4
		160, // 5
		192, // 6
		224, // 7
		256, // 8
		320, // 9
		384, // 10
		448, // 11
		512, // 12
		640, // 13
};

// This maps an arbitrary allocation size to a suitable slot in b2_blockSizes.
struct b2SizeMap
{
	unsigned char values[b2_maxBlockSize + 1];
	b2SizeMap()
	{
		int j = 0;
		values[0] = 0;
		for (int i = 1; i <= b2_maxBlockSize; ++i)
		{
			//  (j < b2_blockSizeCount);
			if (i <= b2_blockSizes[j])
			{
				values[i] = (unsigned char)j;
			}
			else
			{
				++j;
				values[i] = (unsigned char)j;
			}
		}
	}
};

static const b2SizeMap b2_sizeMap;


b2BlockAllocator::b2BlockAllocator()
{
	SQ_ASSERT(b2_blockSizeCount < UCHAR_MAX);

	m_chunkSpace = b2_chunkArrayIncrement;
	m_chunkCount = 0;
	m_chunks = (b2Chunk *)malloc(m_chunkSpace * sizeof(b2Chunk));

	memset(m_chunks, 0, m_chunkSpace * sizeof(b2Chunk));
	memset(m_freeLists, 0, sizeof(m_freeLists));
}

b2BlockAllocator::~b2BlockAllocator()
{
	for (int i = 0; i < m_chunkCount; ++i)
	{
		free(m_chunks[i].blocks);
	}
	free(m_chunks);
}

void *b2BlockAllocator::Allocate(int size)
{
	if (size == 0)
	{
		return nullptr;
	}

	SQ_ASSERT(0 < size);

	if (size > b2_maxBlockSize)
	{
		return malloc(size);
	}

	int index = b2_sizeMap.values[size];

	SQ_ASSERT(0 <= index && index < b2_blockSizeCount);

	if (m_freeLists[index])
	{
		//从空闲列表中获取一个块
		b2Block *block = m_freeLists[index];
		m_freeLists[index] = block->next;
		return block;
	}
	else
	{
		if (m_chunkCount == m_chunkSpace)
		{
			//oh,不够b2Chunk了，在现有的基础上增加空间，每次增长b2_chunkArrayIncrement个b2Chunk
			b2Chunk *oldChunks = m_chunks;
			m_chunkSpace += b2_chunkArrayIncrement;
			m_chunks = (b2Chunk *)malloc(m_chunkSpace * sizeof(b2Chunk));
			memcpy(m_chunks, oldChunks, m_chunkCount * sizeof(b2Chunk));
			memset(m_chunks + m_chunkCount, 0, b2_chunkArrayIncrement * sizeof(b2Chunk));
			free(oldChunks);
		}

		b2Chunk *chunk = m_chunks + m_chunkCount;
		//为chunk分配一段内存
		chunk->blocks = (b2Block *)malloc(b2_chunkSize);

#if defined(_DEBUG)
		memset(chunk->blocks, 0xcd, b2_chunkSize);
#endif

		//得到每个block的大小，所以每个chunk的block大小不是相等的，这是为了减少内存碎片。
		int blockSize = b2_blockSizes[index];
		chunk->blockSize = blockSize;

		//chunk的内存大小除以每个block的大小，得到每个chunk中有多少个block。
		int blockCount = b2_chunkSize / blockSize;

		SQ_ASSERT(blockCount * blockSize <= b2_chunkSize);

		for (int i = 0; i < blockCount - 1; ++i)
		{
			b2Block *block = (b2Block *)((char *)chunk->blocks + blockSize * i);
			b2Block *next = (b2Block *)((char *)chunk->blocks + blockSize * (i + 1));
			block->next = next;
		}
		b2Block *last = (b2Block *)((char *)chunk->blocks + blockSize * (blockCount - 1));
		last->next = nullptr;

		m_freeLists[index] = chunk->blocks->next;
		++m_chunkCount;

		return chunk->blocks;
	}
}

void b2BlockAllocator::Free(void *p, int size)
{
	if (size == 0)
	{
		return;
	}

	SQ_ASSERT(0 < size);

	if (size > b2_maxBlockSize)
	{
		free(p);
		return;
	}

	int index = b2_sizeMap.values[size];

	SQ_ASSERT(0 <= index && index < b2_blockSizeCount);

#if defined(_DEBUG)
	// Verify the memory address and size is valid.
	int blockSize = b2_blockSizes[index];
	bool found = false;
	for (int i = 0; i < m_chunkCount; ++i)
	{
		b2Chunk *chunk = m_chunks + i;
		if (chunk->blockSize != blockSize)
		{
			SQ_ASSERT((char *)p + blockSize <= (char *)chunk->blocks ||
					 (char *)chunk->blocks + b2_chunkSize <= (char *)p);
		}
		else
		{
			if ((char *)chunk->blocks <= (char *)p && (char *)p + blockSize <= (char *)chunk->blocks + b2_chunkSize)
			{
				found = true;
			}
		}
	}

	SQ_ASSERT(found);

	memset(p, 0xfd, blockSize);
#endif

	b2Block *block = (b2Block *)p;
	block->next = m_freeLists[index];
	m_freeLists[index] = block;
}

void b2BlockAllocator::Clear()
{
	for (int i = 0; i < m_chunkCount; ++i)
	{
		free(m_chunks[i].blocks);
	}

	m_chunkCount = 0;
	memset(m_chunks, 0, m_chunkSpace * sizeof(b2Chunk));
	memset(m_freeLists, 0, sizeof(m_freeLists));
}
