/**
 * 二进制字节处理
 * @constructor
 */
export default class Byte {


    protected _xd_: boolean = false;
    protected _pos_: number = 0;
    private _length: number = 0;
    protected _d_: DataView;
    protected _u8d_: Uint8Array;
    public static LITTLE_ENDIAN: string = "bigEndian";
    public static BIG_ENDIAN: string = "littleEndian";
    private static _sysEndian: string = null;

    constructor(data?: Uint8Array | ArrayBuffer) {
        this.endian = Byte.getSystemEndian();
        if (data) {
            this.setData(data);
        }
    }

    get byteData(): Readonly<Uint8Array> {
        return this._u8d_;
    }

    setData(u: Uint8Array | ArrayBuffer): void {
        if (u instanceof ArrayBuffer) {
            u = new Uint8Array(u);
        }
        this._u8d_ = u;
        this._length = u.byteLength;
        this._d_ = new DataView(u.buffer, u.byteOffset, this._length);
    }

    static getSystemEndian() {
        if (!Byte._sysEndian) {
            var buffer = new ArrayBuffer(2);
            new DataView(buffer).setInt16(0, 256, true);
            Byte._sysEndian = (new Int16Array(buffer))[0] === 256 ? Byte.LITTLE_ENDIAN : Byte.BIG_ENDIAN;
        }
        return Byte._sysEndian;
    }

    get endian(): string {
        return this._xd_ ? Byte.LITTLE_ENDIAN : Byte.BIG_ENDIAN;
    }

    set endian(value: string) {
        this._xd_ = (value === Byte.LITTLE_ENDIAN);
    }
    set length(value: number) {
        this._resizeBuffer(value);
        this._length = value;
    }

    get length(): number {
        return this._length;
    }

    private _resizeBuffer(len: number) {
        var newByteView = new Uint8Array(len);
        if (this._u8d_ != null) {
            if (this._u8d_.length <= len)
                newByteView.set(this._u8d_);
            else
                newByteView.set(this._u8d_.subarray(0, len));
        }
        this._u8d_ = newByteView;
        this._d_ = new DataView(newByteView.buffer);
    }

    getString(): string {
        return this.readString();
    }

    readString(): string {
        return this._rUTF(this.getUint16());
    }


    readFloat32Array(numCount: number): Float32Array {
        let p = this._pos_;

        let data: Float32Array;
        let size = numCount * 4;

        //读取Float32地址必须是4字节对其的，即地址为4的倍数
        //但这里记录的数据是紧凑的，不是4字节对齐，所以对于不是4字节对齐
        //则需要复制数据，然后才能解析成浮点型
        if (p % 4 === 0) {
            data = new Float32Array(this._u8d_.buffer);
            data = data.subarray(p >> 2, (p + size) >> 2);
            this._pos_ += size;
        } else {
            // 从原始 buffer 中读取非对齐的数据
            const rawBytes = this.readBuffer(size); // 16 字节，假设包含 4 个 float

            // 创建一个新的对齐的 ArrayBuffer
            const alignedBuffer = new ArrayBuffer(size);
            new Uint8Array(alignedBuffer).set(rawBytes); // 拷贝数据

            // 现在可以安全地用 Float32Array 读取了
            data = new Float32Array(alignedBuffer);

        }
        return data;
    }

    getUint8Array(start: number, len: number) {
        return this.readUint8Array(start, len);
    }

    readUint8Array(start: number, len: number) {
        var end = start + len;
        end = (end > this._length) ? this._length : end;
        var v = new Uint8Array(this._d_.buffer.slice(start, end));
        this._pos_ = end;
        return v;
    }

    getInt16Array(start: number, len: number) {
        return this.readInt16Array(start, len);
    }

    readInt16Array(start: number, len: number) {
        var end = start + len;
        end = (end > this._length) ? this._length : end;
        var v = new Int16Array(this._d_.buffer.slice(start, end));
        this._pos_ = end;
        return v;
    }

    getFloat32(): number {
        return this.readFloat32();
    }

    readFloat32(): number {
        var v = this._d_.getFloat32(this._pos_, this._xd_);
        this._pos_ += 4;
        return v;
    }

    getFloat64(): number {
        return this.readFloat64();
    }

    readFloat64(): number {
        var v = this._d_.getFloat64(this._pos_, this._xd_);
        this._pos_ += 8;
        return v;
    }

    writeFloat32(value: number) {
        this._ensureWrite(this._pos_ + 4);
        this._d_.setFloat32(this._pos_, value, this._xd_);
        this._pos_ += 4;
    }

    writeFloat64(value: number) {
        this._ensureWrite(this._pos_ + 8);
        this._d_.setFloat64(this._pos_, value, this._xd_);
        this._pos_ += 8;
    }

    getInt32(): number {
        return this.readInt32();
    }

    readInt32(): number {
        var float = this._d_.getInt32(this._pos_, this._xd_);
        this._pos_ += 4;
        return float;
    }

    getUint32() {
        return this.readUint32();
    }
    
    readUint32() {
        var v = this._d_.getUint32(this._pos_, this._xd_);
        this._pos_ += 4;
        return v;
    }

    writeInt32(value: number) {
        this._ensureWrite(this._pos_ + 4);
        this._d_.setInt32(this._pos_, value, this._xd_);
        this._pos_ += 4;
    }

    writeUint32(value: number) {
        this._ensureWrite(this._pos_ + 4);
        this._d_.setUint32(this._pos_, value, this._xd_);
        this._pos_ += 4;
    }

    getInt16() {
        return this.readInt16();
    }

    readInt16() {
        // if (this._pos_ + 2 > this._length)
        //     throw "getInt16 error - Out of bounds";
        var us = this._d_.getInt16(this._pos_, this._xd_);
        this._pos_ += 2;
        return us;
    }
    getUint16() {
        return this.readUint16();
    }
    readUint16() {
        // if (this._pos_ + 2 > this._length)
        //     throw "getUint16 error - Out of bounds";
        var us = this._d_.getUint16(this._pos_, this._xd_);
        this._pos_ += 2;
        return us;
    }

    writeUint16(value: number) {
        this._ensureWrite(this._pos_ + 2);
        this._d_.setUint16(this._pos_, value, this._xd_);
        this._pos_ += 2;
    }

    writeInt16(value: number) {
        this._ensureWrite(this._pos_ + 2);
        this._d_.setInt16(this._pos_, value, this._xd_);
        this._pos_ += 2;
    }
    getUint8() {
        return this.readUint8();
    }
    readUint8() {
        // if (this._pos_ + 1 > this._length)
        //     throw "getUint8 error - Out of bounds";
        return this._u8d_[this._pos_++];
    }

    writeUint8(value: number) {
        this._ensureWrite(this._pos_ + 1);
        this._d_.setUint8(this._pos_, value);
        this._pos_++;
    }


    private _rUTF(len: number) {
        var max = this._pos_ + len, c, c2, c3, f = String.fromCharCode;
        var u = this._u8d_;
        var strs = [];
        var n = 0;
        strs.length = 1000;
        while (this._pos_ < max) {
            c = u[this._pos_++];
            if (c < 0x80) {
                if (c != 0)
                    strs[n++] = f(c);
            }
            else if (c < 0xE0) {
                strs[n++] = f(((c & 0x3F) << 6) | (u[this._pos_++] & 0x7F));
            }
            else if (c < 0xF0) {
                c2 = u[this._pos_++];
                strs[n++] = f(((c & 0x1F) << 12) | ((c2 & 0x7F) << 6) | (u[this._pos_++] & 0x7F));
            }
            else {
                c2 = u[this._pos_++];
                c3 = u[this._pos_++];
                const _code = ((c & 0x0F) << 18) | ((c2 & 0x7F) << 12) | ((c3 & 0x7F) << 6) | (u[this._pos_++] & 0x7F);
                if (_code >= 0x10000) {
                    const _offset = _code - 0x10000;
                    const _lead = 0xd800 | (_offset >> 10);
                    const _trail = 0xdc00 | (_offset & 0x3ff);
                    strs[n++] = f(_lead);
                    strs[n++] = f(_trail);
                }
                else {
                    strs[n++] = f(_code);
                }
            }
        }
        strs.length = n;
        return strs.join('');
    }

    getCustomString(len: number) {
        return this.readCustomString(len);
    }

    readCustomString(len: number) {
        var v = "", ulen = 0, c, c2, f = String.fromCharCode;
        var u = this._u8d_;
        while (len > 0) {
            c = u[this._pos_];
            if (c < 0x80) {
                v += f(c);
                this._pos_++;
                len--;
            }
            else {
                ulen = c - 0x80;
                this._pos_++;
                len -= ulen;
                while (ulen > 0) {
                    c = u[this._pos_++];
                    c2 = u[this._pos_++];
                    v += f((c2 << 8) | c);
                    ulen--;
                }
            }
        }
        return v;
    }

    get pos(): number {
        return this._pos_;
    }

    set pos(value: number) {
        this._pos_ = value;
    }

    get bytesAvailable(): number {
        return this._length - this._pos_;
    }

    clear() {
        this._pos_ = 0;
        this.length = 0;
    }

    private __getBuffer() {
        return this._d_.buffer;
    }

    writeUTFBytes(value: string) {
        value = value + "";
        for (var i = 0, sz = value.length; i < sz; i++) {
            var c = value.charCodeAt(i);
            if (c <= 0x7F) {
                //这个范围是使用1个字节存储Unicode
                this.writeUint8(c);
            }
            else if (c <= 0x7FF) {
                //这个范围是使用2个字节存储Unicode
                this._ensureWrite(this._pos_ + 2);
                //0xC0 = 11000000 。表示第一个字节前2为设置1，第三位设置0;
                //表示将Unicode的前5个位存储到第一个字节的后5个位中

                this._u8d_.set([0xC0 | (c >> 6), 0x80 | (c & 0x3F)], this._pos_);
                this._pos_ += 2;
            }
            else if (c >= 0xD800 && c <= 0xDBFF) {
                //UTF-16的存储方式。这个编码方式是使用2个字符表示一个unicode字符的，并且是连在一起的
                i++;
                const c2 = value.charCodeAt(i);
                if (!Number.isNaN(c2) && c2 >= 0xDC00 && c2 <= 0xDFFF) {
                    const _p1 = (c & 0x3FF) + 0x40;
                    const _p2 = c2 & 0x3FF;
                    const _b1 = 0xF0 | ((_p1 >> 8) & 0x3F);
                    const _b2 = 0x80 | ((_p1 >> 2) & 0x3F);
                    const _b3 = 0x80 | ((_p1 & 0x3) << 4) | ((_p2 >> 6) & 0xF);
                    const _b4 = 0x80 | (_p2 & 0x3F);
                    this._ensureWrite(this._pos_ + 4);
                    this._u8d_.set([_b1, _b2, _b3, _b4], this._pos_);
                    this._pos_ += 4;
                }
            }
            else if (c <= 0xFFFF) {
                //使用3个字节存储Unicode
                this._ensureWrite(this._pos_ + 3);
                this._u8d_.set([0xE0 | (c >> 12), 0x80 | ((c >> 6) & 0x3F), 0x80 | (c & 0x3F)], this._pos_);
                this._pos_ += 3;
            }
            else {
                //使用4个字节存储Unicode
                this._ensureWrite(this._pos_ + 4);
                this._u8d_.set([0xF0 | (c >> 18), 0x80 | ((c >> 12) & 0x3F), 0x80 | ((c >> 6) & 0x3F), 0x80 | (c & 0x3F)], this._pos_);
                this._pos_ += 4;
            }
            //UTF8可以支持6个字节存储Unicode的，再大的Unicode的值这里就不存储了，用不到
        }
    }

    writeUTFString(value: string) {
        var tPos = this.pos;
        this.writeUint16(1);
        this.writeUTFBytes(value);
        var dPos = this.pos - tPos - 2;
        this._d_.setUint16(tPos, dPos, this._xd_);
    }

    readUTFString(): string {
        return this.readUTFBytes(this.getUint16());
    }

    getUTFString(): string {
        return this.readUTFString();
    }

    readUTFBytes(len = -1) {
        if (len === 0)
            return "";
        var lastBytes = this.bytesAvailable;
        // if (len > lastBytes)
        //     throw "readUTFBytes error - Out of bounds";
        len = len > 0 ? len : lastBytes;
        return this._rUTF(len);
    }

    getUTFBytes(len = -1) {
        return this.readUTFBytes(len);
    }

    writeByte(value: number) {
        this._ensureWrite(this._pos_ + 1);
        this._d_.setInt8(this._pos_, value);
        this._pos_ += 1;
    }

    readByte(): number {
        // if (this._pos_ + 1 > this._length)
        //     throw "readByte error - Out of bounds";
        return this._d_.getInt8(this._pos_++);
    }

    getByte(): number {
        return this.readByte();
    }

    _ensureWrite(lengthToEnsure: number) {
        if (this._length < lengthToEnsure)
            this.length = lengthToEnsure;
    }

    writeArrayBuffer(arraybuffer: ArrayBuffer, offset = 0, length = 0) {
        if (length == 0)
            length = arraybuffer.byteLength - offset;
        this._ensureWrite(this._pos_ + length);
        var uint8array = new Uint8Array(arraybuffer);
        this._u8d_.set(uint8array.subarray(offset, offset + length), this._pos_);
        this._pos_ += length;
    }

    readBuffer(size: number): Uint8Array {
        let data = this._u8d_.subarray(this._pos_, this._pos_ + size);
        this._pos_ += size;
        return data;
    }

    readDataAddress(): number {
        let pointer = this.readUint32();
        return pointer;
    }
}
