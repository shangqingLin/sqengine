...
import type { IGLGPUShader, IGLGPUVAO, IGSampler } from "../webgl/WebGLGPUObjectDefine";
...
import { WebGL2EXT } from "./WenGL2Define";



function GFXFormatToWebGLType(format: Format, gl: WebGL2RenderingContext): GLenum {
    switch (format) {
...
function GFXFormatToWebGLInternalFormat(format: Format, gl: WebGL2RenderingContext): GLenum {
    switch (format) {
...
function GFXFormatToWebGLFormat(format: Format, gl: WebGL2RenderingContext): GLenum {
    switch (format) {
        case Format.A8: return gl.ALPHA;
        case Format.L8: return gl.LUMINANCE;
        case Format.LA8: return gl.LUMINANCE_ALPHA;
...
export function WebGL2CmdFuncCreateShader(device: WebGL2Device, shaderAssetId: number, programName: number, shader: IGLGPUShader, macroFlags: number): void {
    CmdFuncCreateShader(device, shaderAssetId, programName, shader, macroFlags);
}


function vertexAttribDivisor(device: WebGL2Device, byte: Byte) {
    ...
    device.gl.vertexAttribDivisor(shader.glAttributes[location].glLoc, stride);
    // console.info("run vertexAttribDivisor ",location,stride);
}

function drawElementsInstanced(device: WebGL2Device, byte: Byte) {
    ...
    device.gl.drawElementsInstanced(GLEnumPrimitive[primitive], indexCount, GLEnumMap[type], offset, instanceCount);

    // console.info("drawElementsInstanced instanceCount ", instanceCount, "primitive ", primitive);
...
function deleteVertexArray(device: WebGL2Device, byte: Byte) {
    let vaoId = byte.readUint32();
    let obj: IGLGPUVAO = device.getGpuObject(vaoId) as IGLGPUVAO;
    device.gl.deleteVertexArray(obj.vao);
    device.deleteGpuObject(vaoId);
}

function drawBuffers(device: WebGL2Device, byte: Byte) {
    let num = byte.readInt32();
    let attachments = new Array(num);
    for (let i = 0; i < num; ++i) {
        attachments[i] = device.gl.COLOR_ATTACHMENT0 + i;
    }
    device.gl.drawBuffers(attachments);
}

function createSampler(device: WebGL2Device, byte: Byte) {
    let id = byte.readInt32();
    let obj: IGSampler = Object.create(null) as IGSampler;
    obj.sampler = device.gl.createSampler();
    device.addGpuObject(id, obj);
}

function bindSampler(device: WebGL2Device, byte: Byte) {
    let id = byte.readInt32();
    let texUnit = byte.readInt32();
    if (id == 0) {
        device.gl.bindSampler(texUnit, null);
    } else {
        let obj: IGSampler = device.getGpuObject(id) as IGSampler;
        device.gl.bindSampler(texUnit, obj.sampler);
    }
}

function samplerParameteri(device: WebGL2Device, byte: Byte) {
    let id = byte.readInt32();
    let target = byte.readUint32();
    let value = byte.readUint32();
    let obj: IGSampler = device.getGpuObject(id) as IGSampler;
    device.gl.samplerParameteri(obj.sampler, GLEnumMap[target], GLEnumMap[value]);
}

function deleteSampler(device: WebGL2Device, byte: Byte) {
    let id = byte.readInt32();
    let obj: IGSampler = device.getGpuObject(id) as IGSampler;
    device.gl.deleteSampler(obj.sampler);
    device.deleteGpuObject(id);
}

export function initialize(device: WebGL2Device) {
    initializeBaseAPI(device);
    let context: WebGL2RenderingContext = device.gl;

    GLEnumMap[JSAPAGLEnum.MIN] = context.MIN;
    GLEnumMap[JSAPAGLEnum.MAX] = context.MAX;

    registerCommand(JSAPIMap.vertexAttribDivisor, vertexAttribDivisor);
    registerCommand(JSAPIMap.drawElementsInstanced, drawElementsInstanced);
    registerCommand(JSAPIMap.createVertexArray, createVertexArray);
    registerCommand(JSAPIMap.bindVertexArray, bindVertexArray);
    registerCommand(JSAPIMap.deleteVertexArray, deleteVertexArray);
    registerCommand(JSAPIMap.drawBuffers, drawBuffers);
    registerCommand(JSAPIMap.createSampler, createSampler);
    registerCommand(JSAPIMap.deleteSampler, deleteSampler);
    registerCommand(JSAPIMap.samplerParameteri, samplerParameteri);
    registerCommand(JSAPIMap.bindSampler, bindSampler);
