...
import { NodeEventType, Node } from "../scene";
import { UIPackage } from "./assest/UIPackage";
import { AlignType, ChildrenRenderOrder, ListLayoutType, ListSelectionMode, VertAlignType } from "./common/FieldTypes";
import { GObjectPool } from "./common/GObjectPool";
...
export class GList extends GComponent {
    ...
    public override dispose(): void {
        Application.ins.timer.clear(this, this._refreshVirtualList);
        this._pool.clear();
        super.dispose();
    }
    ...
    public override addChildAt(child: GObject, index: number): GObject {
        ...
        child.on(NodeEventType.MOUSE_CLICK, this.onClickItem, this);
    ...
    public override removeChildAt(index: number, dispose?: boolean): GObject {
        var child: GObject = super.removeChildAt(index, dispose);
        if (!dispose)
            child.off(NodeEventType.MOUSE_CLICK, this.onClickItem, this);
    ...
    private onClickItem(evt: EventMouse): void {
        if (this._scrollPane && this._scrollPane.isDragged)
            return;

        var item: GObject = GObject.cast(<Node>evt.target);
    ...
    protected dispatchItemEvent(item: GObject, evt: EventMouse): void {
        this._node.fire(FUIEvent.CLICK_ITEM, item, evt);
    }

    private setSelectionOnEvent(item: GObject, evt: EventMouse): void {
        ...
        if (this._selectionMode == ListSelectionMode.Single) {
            ...
            else if (this._selectionMode == ListSelectionMode.Multiple_SingleClick) {
    ...
    public override getSnappingPosition(xValue: number, yValue: number, resultPoint?: Vec2): Vec2 {
        if (this._virtual) {
    ...
    public override getFirstChildInView(): number {
    ...
    private _setVirtual(loop: boolean): void {
        if (!this._virtual) {
            ...
            this._node.on(FUIEvent.SCROLL, this, this.__scrolled);
            this.setVirtualListChangedFlag(true);
        }
    }
    ...
    private setVirtualListChangedFlag(layoutChanged: boolean): void {
        ...
        Application.ins.timer.callLater(this, this._refreshVirtualList);
    }
    ...
    private handleScroll1(forceUpdate: boolean): boolean {
        ...
        if (curIndex > 0 && this.numChildren > 0 && this._container.transform.y <= 0 && this.getChildAt(0).y > -this._container.transform.y)//最后一页没填满！
            return true;
        else
            return false;
    }
    ...
    private handleScroll2(forceUpdate: boolean): boolean {
        ...
        if (curIndex > 0 && this.numChildren > 0 && this._container.transform.x <= 0 && this.getChildAt(0).x > - this._container.transform.x)//最后一页没填满！
            return true;
        else
            return false;
    }
