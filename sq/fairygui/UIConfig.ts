import { AssetManager, Font } from "../assets";
import { Color } from "../core";
import { Layers } from "../scene";
import { ScrollBarDisplayType } from "./common/FieldTypes";
...
export class UIConfig {
    ...
    public static defaultUILayer: number = Layers.UI;
}

let _fontRegistry: { [index: string]: Font } = {};
export function registerFont(name: string, font?: Font | string): void {
    if (font instanceof Font)
        _fontRegistry[name] = font;
    else {
        AssetManager.getInstance().load(font || name, (err: Error | null, asset: Font) => {
