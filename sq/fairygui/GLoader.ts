import { SpriteSizeMode, SpriteType, UIContentComponent } from "../2d";
import { Color, Vec2 } from "../core";
import { PackageItem } from "./assest/PackageItem";
import { AlignType, FillMethod, FillOrigin, LoaderFillType, ObjectPropID, PackageItemType, VertAlignType } from "./common/FieldTypes";
import { MovieClip } from "./display/MovieClip";
import { GComponent } from "./GComponent";
import { GObject } from "./GObject";
import { Node } from "../scene";
import { Asset, AssetManager, ImageAsset, SpriteFrame, Texture2d } from "../assets";
...
export class GLoader extends GObject {
    ...
    /**
     * 设置装载器是否根据内容的大小自动调整自身的大小。例如，如果图片的大小是100x100，则装载器的大小自动变成100x100。
     * 如果设置为false,则大小节点上设置的width和height
     */
    private _autoSize: boolean;

    /**
     * 当需要显示的图片或动画与装载器设置的大小（width和height）不相同时使用的缩放策略。
     */
    ...
    private _containerUITrans: UIContentComponent;
    ...
    public constructor() {
        ...
        this._content = this._containerUITrans = this._container.addComponent(MovieClip);
    ...
    public set texture(value: SpriteFrame) {
        ...
        this._content.type = SpriteType.SIMPLE;
        if (value != null) {
            ...
            if (!contentItem.asset) {
                ...
                if (this._content.fillMethod == 0) {
                    if (contentItem.scale9Grid)
                        this._content.type = SpriteType.SLICED;
                    else if (contentItem.scaleByTile)
                        this._content.type = SpriteType.TILED;
                    else
                        this._content.type = SpriteType.SIMPLE;
                }
                else {
                    this._content.type = SpriteType.FILLED;
                }
                this.updateLayout();
            }
        }
    ...
    protected loadExternal(): void {
        ...
        if (this._url.startsWith('data:image/')) {
            ...
            AssetManager.getInstance().load(this._url, callback);
        }
    }
    ...
    private updateLayout(): void {
        ...
        if (this._autoSize) {
            ...
            this._container.transform.setPosition(pivotCorrectX, pivotCorrectY);
            if (this._content2) {
        ...
        if (this._verticalAlign == VertAlignType.Top)
            ...
            ny = this._height - ch;
        ny = -ny;
        this._container.transform.setPosition(pivotCorrectX + nx, pivotCorrectY + ny);
    }
