import { SpriteComponent, SpriteFillType, SpriteType, UIContentComponent } from "../../2d";
import { Vec2 } from "../../core";
import { FillMethod, FillOrigin, FlipType } from "../common/FieldTypes";

export class Image extends SpriteComponent {
    ...
    public set flip(value: FlipType) {
        if (this._flip != value) {
            ...
            if (sx != 1 || sy != 1) {
                let uiTrans = this.node.getCompoentInherit(UIContentComponent);
                uiTrans.anchorX = 0.5;
                uiTrans.anchorY = 0.5;
            }
            this.node.transform.scaleX = sx;
            this.node.transform.scaleY = sy;
        }
    }
        ...
        if (this._fillMethod != value) {
            ...
            if (this._fillMethod != 0) {
                ...
                if (this._fillMethod <= 3)
                    ...
                    this.fillType = SpriteFillType.RADIAL;

                this.fillCenter = new Vec2(0.5, 0.5);
                this.setupFill();
            }
            else {
                this.type = SpriteType.SIMPLE;
            }
        }
    }
