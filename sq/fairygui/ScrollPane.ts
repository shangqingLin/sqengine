...
import { ScrollBarDisplayType, ScrollType } from "./common/FieldTypes";
...
import { MaskComponent, UIContentComponent } from "../2d";
import { CommonUtils, math, Rect, Vec2 } from "../core";
import { Node, NodeEventType } from "../scene";
import { FUIEvent } from "./FUIEvent";
import { Application } from "../framework/Application";
import { EventMouse } from "../input/Events";
import { screen, systemInfo } from "../platform";
...
export class ScrollPane extends Component {
    ...
    /**
     * 添加了这个组件的GCompnent中的Container节点
     * ScrollPanel就是对这个Container进行滚动的
     */
    private _container: Node;
    private _maskContainer: Node;
    private _maskContainerUITrans: UIContentComponent;
    ...
    /**
     * 启动还是关闭滚动功能，如果设置为false，则不能滚动ScrollPanel
     */
    ...
    /**
     * 内容区域的大小，当_contentSize超出_viewSize时就会显示滚动条，就可以滚动内容了
     */
    private _contentSize: Vec2;

    /**
     * 存储可以滚动的区域的大小。即 _contentSize - _viewSize 的大小
     */
    ...
    protected override onInitialize(): void {
        ...
        // this._maskContainerUITrans = this._maskContainer.addComponent(UIContentComponent);
        // this._maskContainerUITrans.setAnchorPoint(0, 1);
        o.node.addChild(this._maskContainer);
        ...
        o.on(NodeEventType.MOUSE_DOWN, this.onTouchBegin, this);
        o.on(NodeEventType.MOUSE_WHEEL, this.onMouseWheel, this);
    }

    private onMaskTypeChange(): void {
        this._maskContainerUITrans = (this._maskContainer.addComponent(MaskComponent) as MaskComponent).subComp;
        this._maskContainerUITrans.setAnchorPoint(0, 1);
    ...
    public get scrollingPosX(): number {
        return math.clamp(-this._container.transform.x, 0, this._overlapSize.x);
    }

    public get scrollingPosY(): number {
        return math.clamp(-(-this._container.transform.y), 0, this._overlapSize.y);
    }
    ...
    public isChildInView(obj: GObject): boolean {
        if (this._overlapSize.y > 0) {
            var dist: number = obj.y + (-this._container.transform.y);
            if (dist < -obj.height || dist > this._viewSize.y)
                return false;
        }

        if (this._overlapSize.x > 0) {
            dist = obj.x + this._container.transform.x;
    ...
    public lockHeader(size: number): void {
        ...
        let cx: number = this._container.transform.x;
        let cy: number = -this._container.transform.y;
    ...
    public adjustMaskContainer(): void {
        ...
        if (!this._maskContainerUITrans) return;
        ...
        if (o._customMask)
            this._maskContainer.transform.setPosition(mx + o._alignOffset.x, -o._alignOffset.y);
        else
            this._maskContainer.transform.setPosition(o._pivotCorrectX + mx + o._alignOffset.x, o._pivotCorrectY - o._alignOffset.y);
    }

    /**
     * 设置可视区域的大小。如果ContentSize超出这个区域的大小就会显示滚动条了
     * @param aWidth 
     * @param aHeight 
     */
    ...
    public changeContentSizeOnScrolling(deltaWidth: number, deltaHeight: number,
        ...
        if (this._tweening === 1) {
            ...
            //重新调整起始位置，确保能够顺滑滚下去
            if (deltaPosX != 0) {
                this._container.transform.setPosition(this._container.transform.x - deltaPosX, this._container.transform.y);
                this._tweenStart.x -= deltaPosX;
                this._xPos = -this._container.transform.x;
            }
            if (deltaPosY != 0) {
                this._container.transform.setPosition(this._container.transform.x, this._container.transform.y + deltaPosY);
                this._tweenStart.y -= deltaPosY;
                this._yPos = -(-this._container.transform.y);
            }
        }
            ...
            if (deltaWidth != 0 && isRightmost) {
                this._xPos = this._overlapSize.x;
                this._container.transform.setPosition(-this._xPos, this._container.transform.y);
            }

            if (deltaHeight != 0 && isBottom) {
                this._yPos = this._overlapSize.y;
                this._container.transform.setPosition(this._container.transform.x, this._yPos);
            }
        }
    ...
    private handleSizeChanged(): void {
        ...
        if (this._vtScrollBar) {
            ...
            if (this._contentSize.y == 0)
                this._vtScrollBar.setDisplayPerc(0); //内容高度为0，则隐藏滚动条内容按钮
            else
        ...
        if (this._refreshBarAxis == "x")
            this._container.transform.setPosition(math.clamp(this._container.transform.x, -max, this._headerLockedSize),
                -math.clamp((-this._container.transform.y), -this._overlapSize.y, 0));
        else
            this._container.transform.setPosition(math.clamp(this._container.transform.x, -this._overlapSize.x, 0),
                -math.clamp((-this._container.transform.y), -max, this._headerLockedSize));
    ...
    private refresh(dt?: number): void {
        ...
        this._owner.node.fire(FUIEvent.SCROLL, this._owner);
    ...
    private refresh2() {
        if (this._aniFlag === 1 && !this._dragged) {
            ...
            if (this._overlapSize.x > 0)
                ...
                if (this._container.transform.x !== 0)
                    this._container.transform.setPosition(0, this._container.transform.y);
                posX = 0;
            }
            ...
            if (this._overlapSize.y > 0)
                ...
                if (this._container.transform.y != 0)
                    this._container.transform.setPosition(this._container.transform.x, 0);
                posY = 0;
            }

            if (posX !== this._container.transform.x || posY !== (-this._container.transform.y)) {
                this._tweenDuration.x = this._tweenDuration.y = TWEEN_TIME_GO;
                this._tweenStart.x = this._container.transform.x;
                this._tweenStart.y = (-this._container.transform.y);
            ...
            this._container.transform.setPosition(Math.floor(-this._xPos), -Math.floor(-this._yPos));
    ...
    private onTouchBegin(evt: EventMouse): void {
        if (!this._touchEffect)
            return;

        if (this._tweening != 0) {
            //如果当前处于滚动状态，则停止滚动。由用户在这里拖动滚动就行
            this.killTween();
            this._dragged = true;
        }
        else
        ...
        Vec2.temp1Vec2.set(evt.worldX, -evt.worldY);
        var pt: Vec2 = Vec2.temp1Vec2;

        this._owner.on(NodeEventType.MOUSE_MOVE, this.onTouchMove, this);
        this._owner.on(NodeEventType.MOUSE_UP, this.onTouchEnd, this);

        // var pt: Vec2 = this._owner.globalToLocal(evt.worldX, evt.worldY, Vec2.temp2Vec2);

        // console.info("world pos ", evt.worldX, evt.worldY, pt.x, pt.y);

        this._containerPos.x = this._container.transform.x;
        this._containerPos.y = -this._container.transform.y;
        this._beginTouchPos.set(pt.x, pt.y);
        this._lastTouchPos.set(pt.x, pt.y);
        this._lastTouchGlobalPos.set(evt.localX, evt.localY);
    ...
    private onTouchMove(evt: EventMouse): void {

        if (!CommonUtils.isDefine(this._owner.node))
            return;
        ...
        if (sv) {
            ...
            if (newPosY > 0) {
                if (!this._bouncebackEffect)
                    ...
                    this._container.transform.setPosition(this._container.transform.x, - Math.floor(Math.min(newPosY * 0.5, this._header.maxHeight)));
                else
                    this._container.transform.setPosition(this._container.transform.x, - Math.floor(Math.min(newPosY * 0.5, this._viewSize.y * PULL_RATIO)));
            }
            else if (newPosY < -this._overlapSize.y) {
                if (!this._bouncebackEffect)
                    this._container.transform.setPosition(this._container.transform.x, this._overlapSize.y);
                else if (this._footer && this._footer.maxHeight > 0)
                    this._container.transform.setPosition(this._container.transform.x, - Math.floor(Math.max((newPosY + this._overlapSize.y) * 0.5, -this._footer.maxHeight) - this._overlapSize.y));
                else
                    this._container.transform.setPosition(this._container.transform.x, - Math.floor(Math.max((newPosY + this._overlapSize.y) * 0.5, -this._viewSize.y * PULL_RATIO) - this._overlapSize.y));
            }
            else
                this._container.transform.setPosition(this._container.transform.x, - newPosY);
        }
        ...
        if (sh) {
            ...
            if (newPosX > 0) {
                if (!this._bouncebackEffect)
                    ...
                    this._container.transform.setPosition(Math.floor(Math.min(newPosX * 0.5, this._header.maxWidth)), this._container.transform.y);
                else
                    this._container.transform.setPosition(Math.floor(Math.min(newPosX * 0.5, this._viewSize.x * PULL_RATIO)), this._container.transform.y);
            }
            else if (newPosX < 0 - this._overlapSize.x) {
                if (!this._bouncebackEffect)
                    this._container.transform.setPosition(-this._overlapSize.x, this._container.transform.y);
                else if (this._footer && this._footer.maxWidth > 0)
                    this._container.transform.setPosition(Math.floor(Math.max((newPosX + this._overlapSize.x) * 0.5, -this._footer.maxWidth) - this._overlapSize.x), this._container.transform.y);
                else
                    this._container.transform.setPosition(Math.floor(Math.max((newPosX + this._overlapSize.x) * 0.5, -this._viewSize.x * PULL_RATIO) - this._overlapSize.x), this._container.transform.y);
            }
            else
                this._container.transform.setPosition(newPosX, this._container.transform.y);
        }
        ...
        */
        var deltaGlobalPositionX: number = this._lastTouchGlobalPos.x - evt.worldX;
        var deltaGlobalPositionY: number = this._lastTouchGlobalPos.y - evt.worldY;
        if (deltaPositionX != 0)
        ...
        this._lastTouchPos.set(pt.x, pt.y);
        this._lastTouchGlobalPos.set(evt.worldX, evt.worldY);
        // this._lastMoveTime = now;

        //同步更新pos值
        if (this._overlapSize.x > 0)
            this._xPos = math.clamp(-this._container.transform.x, 0, this._overlapSize.x);
        if (this._overlapSize.y > 0)
            this._yPos = math.clamp(-(-this._container.transform.y), 0, this._overlapSize.y);

        //循环滚动特别检查
        if (this._loop != 0) {
            newPosX = this._container.transform.x;
            newPosY = (-this._container.transform.y);
            if (this.loopCheckingCurrent()) {
                this._containerPos.x += this._container.transform.x - newPosX;
                this._containerPos.y += (-this._container.transform.y) - newPosY;
            }
        }
        ...
        this._owner.node.fire(FUIEvent.SCROLL);
    }
    ...
    private onTouchEnd(evt: FUIEvent): void {
        ...
        this._owner.off(NodeEventType.MOUSE_MOVE, this.onTouchMove, this);
        this._owner.off(NodeEventType.MOUSE_UP, this.onTouchEnd, this);
        ...
        else if (this._container.transform.x < -this._overlapSize.x) {
        ...
        else if ((-this._container.transform.y) < -this._overlapSize.y) {
        ...
        if (flag) {
            ...
            if (this._tweenChange.x < -UIConfig.touchDragSensitivity || this._tweenChange.y < -UIConfig.touchDragSensitivity) {
                ...
                this._owner.node.fire(FUIEvent.PULL_DOWN_RELEASE), this._owner;
                this._refreshEventDispatching = false;
            }
            else if (this._tweenChange.x > UIConfig.touchDragSensitivity || this._tweenChange.y > UIConfig.touchDragSensitivity) {
                this._refreshEventDispatching = true;
                this._owner.node.fire(FUIEvent.PULL_UP_RELEASE, this._owner);
                this._refreshEventDispatching = false;
            }
            ...
            if (!this._inertiaDisabled) {
                ...
                var elapsed: number = Application.ins.timer.dt * frameRate - 1;
                if (elapsed > 1) {
                    var factor: number = Math.pow(0.833, elapsed);
                    this._velocity.x = this._velocity.x * factor;
                    this._velocity.y = this._velocity.y * factor;
    ...
    private onMouseWheel(evt: EventMouse) {
        ...
        let delta = evt.originEvent.mouseWheelDelta > 0 ? -1 : 1;
    ...
    private updateScrollBarPos(): void {
        if (this._vtScrollBar)
            this._vtScrollBar.setScrollPerc(this._overlapSize.y == 0 ? 0 : math.clamp(this._container.transform.y, 0, this._overlapSize.y) / this._overlapSize.y);

        if (this._hzScrollBar)
            this._hzScrollBar.setScrollPerc(this._overlapSize.x == 0 ? 0 : math.clamp(-this._container.transform.x, 0, this._overlapSize.x) / this._overlapSize.x);
    ...
    public updateScrollBarVisible(): void {
        if (this._vtScrollBar) {
            if (this._viewSize.y <= this._vtScrollBar.minSize || this._vScrollNone)
                this._vtScrollBar.node.visible = false;
            else
        ...
        if (this._hzScrollBar) {
            if (this._viewSize.x <= this._hzScrollBar.minSize || this._hScrollNone)
                this._hzScrollBar.node.visible = false;
            else
    ...
    private loopCheckingNewPos(value: number, axis: AxisType): number {
        ...
        if (changed) {
            if (axis == "x")
                this._container.transform.setPosition(-Math.floor(pos), this._container.transform.y);
            else
                this._container.transform.setPosition(this._container.transform.x, Math.floor(pos));
        }
    ...
    private alignByPage(pos: number, axis: AxisType, inertialScrolling: boolean): number {
        ...
        if (pos > 0)
            ...
            var change: number = inertialScrolling ? (pos - this._containerPos[axis]) : (pos - (axis == "x" ? this._container.transform.x : (-this._container.transform.y)));
    ...
    private updateTargetAndDuration2(pos: number, axis: AxisType): number {
        ...
        if (pos > 0)
            ...
            //以屏幕像素为基准
            var isMobile: boolean = systemInfo.isMobile;
            var v2: number = Math.abs(v) * this._velocityScale;
            const winSize = screen.getScreenSize();
    ...
    private startTween(type: number): void {
        Application.ins.timer.loop(1, this, this.update);
    ...
    private killTween(): void {
        ...
        {
            this._container.transform.setPosition(this._tweenStart.x + this._tweenChange.x, -(this._tweenStart.y + this._tweenChange.y));
            this._owner.node.fire(FUIEvent.SCROLL, this._owner);
        }

        this._tweening = 0;
        Application.ins.timer.clear(this, this.update);
        this.updateScrollBarVisible();

        this._owner.node.fire(FUIEvent.SCROLL_END, this._owner);
    }
    ...
    private checkRefreshBar(): void {
        ...
        var pos: number = (this._refreshBarAxis == "x" ? this._container.transform.x : (-this._container.transform.y));
        if (this._header) {
            if (pos > 0) {
                this._header.node.visible = true;
                var pt: Vec2 = s_vec2;
                ...
                this._header.node.visible = false;
            }
        }
        ...
        if (this._footer) {
            ...
            if (pos < -max || max == 0 && this._footerLockedSize > 0) {
                this._footer.node.visible = true;
                ...
                this._footer.node.visible = false;
            }
        }
    }
    ...
    private update(): boolean {
        ...
        let dt: number = Application.ins.timer.dt;
        var nx: number = this.runTween("x", dt);
        var ny: number = this.runTween("y", dt);

        // console.info("update", dt, nx, ny);

        this._container.transform.setPosition(nx, -ny);
    ...
    private runTween(axis: AxisType, dt: number): number {
        ...
        if (this._tweenChange[axis] != 0) {
            ...
            newValue = (axis == "x" ? this._container.transform.x : (-this._container.transform.position.y));
