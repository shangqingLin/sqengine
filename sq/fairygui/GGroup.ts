...
import { GroupLayoutType } from "./common/FieldTypes";
...
export class GGroup extends GObject {
        ...
        if (this._updating == 0 && this._parent) {
            ...
            if (!this._boundsChanged) {
                ...
                if (this._layout != GroupLayoutType.None)
                    Application.ins.timer.callLater(this, this._ensureBoundsCorrect);
            }
        }
    }
    ...
    private updateBounds(): void {
        Application.ins.timer.clear(this, this._ensureBoundsCorrect);
    ...
    public override moveChildren(dx: number, dy: number): void {
    ...
    protected override resizeChildren(dw: number, dh: number): void {
    ...
    public override handleAlphaChanged(): void {
    ...
    protected override handleGroupVisibleChanged(): void {
        if (!this._parent)
            return;
...
//与GObject有循环依赖，不得不这样做
//@ts-ignore。fgui库中就行写循环依赖的，不知道他是怎么编译通过的
