...
export class GTree extends GList {
    ...
    private createCell(node: GTreeNode): void {
        ...
        if (cc) {
            cc.on(FUIEvent.STATUS_CHANGED, this, this.__expandedStateChanged);
        ...
        if (node.isFolder)
            node._cell.on(NodeEventType.MOUSE_DOWN, this.__cellMouseDown, this);
    ...
    private __cellMouseDown(evt: EventMouse): void {
        var node: GTreeNode = GObject.cast(<Node>evt.target)._treeNode;
    ...
    protected override dispatchItemEvent(item: GObject, evt: EventMouse): void {
