...
export enum PhyiscsDrawFlag {
    ...
    drawMass = 1 << 6,
    drawContacts = 1 << 7,
    drawContactNormals = 1 << 8,
    drawContactImpulses = 1 << 9,
    drawContactFeatures = 1 << 10,
    drawFrictionImpulses = 1 << 11,
    drawIslands = 1 << 12
};

export class ContactManifold {
    private _temp: Vec2 = new Vec2();

    //Native端的数据指针    
    private dataAdress: number;
   ...
   * 当前与之碰撞的Node
   */
    public readonly collisoinNode: Node;

    get normal(): Readonly<Vec2> {
        this._temp.x = window.Module.getValue(this.dataAdress, "float");
        this._temp.y = window.Module.getValue(this.dataAdress + 4, "float");
        return this._temp;
    }

    get pointCount(): number {
        return window.Module.getValue(this.dataAdress + 8, "i32");
    }

    get worldPointA(): Readonly<Vec2> {
        this._temp.x = window.Module.getValue(this.dataAdress + 12, "float");
        this._temp.y = window.Module.getValue(this.dataAdress + 16, "float");
        return this._temp;
    }

    get worldPointB(): Readonly<Vec2> {
        this._temp.x = window.Module.getValue(this.dataAdress + 52, "float");
        this._temp.y = window.Module.getValue(this.dataAdress + 56, "float");
        return this._temp;
    }
